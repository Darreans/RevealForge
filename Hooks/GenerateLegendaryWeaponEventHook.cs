                ...
                try
                {
                    _eventQuery = _entityManager.CreateEntityQuery(new EntityQueryDesc
                    {
                        All = new ComponentType[] {
                            ComponentType.ReadOnly<ProjectM.Network.GenerateLegendaryWeaponDebugEvent>(),
            ...
            try
            {
                ...
                foreach (var entity in entities)
                {
                    if (!_entityManager.Exists(entity) ||
                        ...
                        !_entityManager.HasComponent<ProjectM.Network.GenerateLegendaryWeaponDebugEvent>(entity))
                    {
                        continue;
                    }
                    ...
                    ProjectM.Network.GenerateLegendaryWeaponDebugEvent eventData = _entityManager.GetComponentData<ProjectM.Network.GenerateLegendaryWeaponDebugEvent>(entity);

                    PrefabGUID weaponGuid = eventData.ItemPrefabGuid;
                    float power = eventData.Power;
                    ...
                    string statsString = $"Power {power:F2}";
                    string action = $"generated {weaponName}";
                    string details = $"({statsString})";
